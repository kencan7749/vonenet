...
class MN(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, kernel_size_t=None, stride=4,  fps=30):
        super().__init__()
        self.in_channels = in_channels
        self.out_channels = out_channels
        self.kernel_size = (kernel_size, kernel_size)
        self.stride = stride
        if kernel_size_t == None:
            self.kernel_size_t = kernel_size
        else:
            self.kernel_size_t = kernel_size_t
        self.padding = (kernel_size_t //2, kernel_size // 2, kernel_size // 2)
        # Param instatiations
        self.weight = torch.zeros((out_channels, in_channels, kernel_size_t, kernel_size, kernel_size))
        self.fps = fps

    def forward(self, x):
        return F.conv3d(x, self.weight, None, self.stride, self.padding)

    def initialize(self, sf, tf, theta, sigx, sigy, phase):
        random_channel = torch.randint(0, self.in_channels, (self.out_channels,))
        for i in range(self.out_channels):
            #first_kernel = gabor_kernel(frequency=sf[i], sigma_x=sigx[i], sigma_y=sigy[i],
             #                                                theta=theta[i], offset=phase[i], ks=self.kernel_size[0])
            #create 3D gabor
            if self.fps < self.kernel_size_t:
                ratio = self.kernel_size_t // self.fps + 1
            else:
                ratio = 1
            bins = np.linspace(0, 1*ratio, self.fps * ratio)
            t_offset = bins[:self.kernel_size_t]
            
            gbs = torch.cat([gabor_kernel(frequency=sf[i], sigma_x=sigx[i], sigma_y=sigy[i],
                                    theta=theta[i], offset= tf[i]*2*np.pi*t_offset[jj] +phase[i], ks=self.kernel_size[0])[np.newaxis] for jj in range(self.kernel_size_t)])
           
            #self.weight[i, random_channel[i]] = gbs
            self.weight[i, :] = torch.cat([gbs[np.newaxis] * self.in_channels])
        self.weight = nn.Parameter(self.weight, requires_grad=False)
...
class MNBlock(nn.Module):
    def __init__(self, sf, tf,  theta, sigx, sigy, phase,
                 k_exc=25, noise_mode=None, noise_scale=1, noise_level=1,
                 simple_channels=128, complex_channels=128, ksize=25, stride=4, input_size=224, ksize_t = 16, fps=30):
        super().__init__()
        ...
        self.tf = tf
        self.ksize_t = ksize_t
        self.fps = fps

        self.simple_conv_q0 = MN(self.in_channels, self.out_channels, ksize, stride = stride, kernel_size_t=self.ksize_t, fps=self.fps)
        self.simple_conv_q1 = MN(self.in_channels, self.out_channels, ksize, stride = stride, kernel_size_t=self.ksize_t, fps=self.fps)
        self.simple_conv_q0.initialize(sf=self.sf, tf = self.tf, theta=self.theta, sigx=self.sigx, sigy=self.sigy,
                                       phase=self.phase)
        self.simple_conv_q1.initialize(sf=self.sf, tf = self.tf, theta=self.theta, sigx=self.sigx, sigy=self.sigy,
                                       phase=self.phase + np.pi/2)
    ...
    def forward(self, x):
        # Gabor activations [Batch, out_channels, H/stride, W/stride]
        x = self.MotionEnergy(x)
        
        return x.mean(2) #<- output should be similar to image feature 

    def MotionEnergy(self, x):
        s_q0 = self.simple_conv_q0(x)
        s_q1 = self.simple_conv_q1(x)
        c = self.complex(torch.sqrt(s_q0[:, self.simple_channels:, :, :] ** 2 +
                                    s_q1[:, self.simple_channels:, :, :] ** 2) / np.sqrt(2))
